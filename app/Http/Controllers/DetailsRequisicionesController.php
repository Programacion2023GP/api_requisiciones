   ...
   public function ordenCompra(Request $request)
{
    try {
        ...
        if ($requisiciones->isEmpty()) {
            return ApiResponse::error('No se encontraron requisiciones', 404);
        }

        foreach ($requisiciones as $requisicion) {
            $requisicion->Proveedor = $request->Proveedor;
            $requisicion->save();
        }

        return ApiResponse::success($requisiciones, 'Proveedor asignado a todas las requisiciones');
    } catch (Exception $e) {
        return ApiResponse::error($e->getMessage(), 500);
    }
}
